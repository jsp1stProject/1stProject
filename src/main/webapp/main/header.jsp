        ...
        <div class="d-flex justify-content-end flex-fill">
	        ...
	        <div class="d-flex justify-content-end align-self-baseline">
	        	<div class="d-flex" id="nav-wide-right">
					...
					<c:if test="${fn:contains(pageContext.request.requestURI, 'fes') || not empty event }">
						<div class="flex-shrink-0 btn-square rounded-circle cartbtn" onclick="location.href='${(not empty event)?'../event/cart.do':'../fes/fes_cart_list.do'}'">
						<span class="cart_num">
							<c:choose>
								<c:when test="${not empty event && not empty eventcart}">${eventcart}</c:when>
								<c:otherwise>0</c:otherwise>
							</c:choose>
						</span> <!--100 이상은 99+로 표시-->
	            ...
	            <button type="button" class="navbar-toggler me-2 ms-2" data-bs-toggle="collapse" data-bs-target="#navbarCollapse">
